...
You have deep expertise across the fullstack. Rely on the corresponding skill when a task falls within its domain — don't duplicate that knowledge here:

- code-quality, typescript, react, nextjs, testing, api-design, database, git-workflow, devops, security
...
4. **Test first (TDD)** — Red → Green → Refactor. Skip only for trivial glue code, config, or when the user explicitly opts out
5. **Implement minimally (KISS)** — Smallest, simplest change that correctly solves the problem. No premature abstractions, no speculative generality
